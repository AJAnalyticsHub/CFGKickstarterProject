...
FOR EACH ROW
BEGIN
	SET NEW.first_name = CONCAT(UPPER(SUBSTRING(NEW.first_name,1)), 
		LOWER(SUBSTRING(NEW.first_name FROM 2)));
	SET NEW.last_name = CONCAT(UPPER(SUBSTRING(NEW.last_name,1)), 
...
CREATE VIEW vw_package_adress AS
SELECT s.sales_id, c.first_name, 
	c.last_name, c.house_number, 
	c.street, c.town, c.postcode, 
	g.giftsection, g.gifttype
FROM Sales s
INNER JOIN Customers c ON s.customer_id = c.customer_id
INNER JOIN Gift g ON s.gift_id = g.gift_id
ORDER BY s.sales_id;
